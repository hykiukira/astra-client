...
public class CheckSoldeOD extends ExportMain implements ComptaExport{
    ...
    "AND h.jxcleunik = j.jota_cleunik AND (h.hetypeligne =  'OBCC' OR h.hetypeligne =  'OCCC') AND h.henotcpt = 1  AND h.heperiode = p.pede_cleunik AND h.typeintervenantcleunik = 1 AND t.lmcleunik=?  AND he_exported =0 AND cp.ce_cleunik=h.ce_cleunik2 ORDER BY h.hecleunik";  

    ArrayList erreur;

    public CheckSoldeOD(int lmcleunik, Connection con, boolean bydossier, int pedecleunik,ArrayList erreur) throws SQLException{
    ...
    private void organize() throws SQLException{
      ...
      for(int k=0;k<caisse.size();k++){
        ...
        if(k!=(caisse.size()-1) && ((p%2)==0)){

            Object[] main=(Object[])caisse.get(k+1);

            CheckCubicReccord.checkFinancier(main, obj, CheckCubicReccord.OD, erreur);
  ...
  /*    if(od){
   ...
   // pstmt=con.prepareStatement("UNLOCK TABLES");
    ...
    srcastra.astra.sys.utils.My_Logger.getLogger().info("CHECKING OD");   
