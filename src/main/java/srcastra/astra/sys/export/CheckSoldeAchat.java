 ...
 */
public class CheckSoldeAchat extends ExportMain implements ComptaExport{
    ...
    "AND h.henotcpt = 1  AND h.heperiode = p.pede_cleunik AND h.typeintervenantcleunik = 2 AND t.lmcleunik=? AND he_exported =0 AND h.jxcleunik=? AND h.henumpiece=? AND cp.ce_cleunik=h.ce_cleunik2 ORDER BY h.hecleunik";
    ...
    ArrayList erreur;
    public CheckSoldeAchat(int lmcleunik, Connection con, boolean bydossier, int pedecleunik,ArrayList erreur) throws SQLException{
    ...
    public ArrayList execute() throws SQLException{
       // PreparedStatement pstmt=con.prepareStatement("LOCK TABLES historique2 WRITE");
        //pstmt.execute();
         srcastra.astra.sys.utils.My_Logger.getLogger().info("CHECKING BUY");   
   ...
   /*     if(fournKeyH!=null){
     ...
     /*   for(Enumeration enu=dossierTMP.keys();enu.hasMoreElements();){
      ...
      //    System.out.println("check des achats");  
        for(int i=0;i<comptArray.size();i++){
           ArrayList tmp=(ArrayList)comptArray.get(i);
   ...
   //    printContrepartie(dossierA);
    ...
    private ArrayList cubicFormat(ArrayList array,Hashtable client)throws SQLException{
        ...
        if(array!=null){ 
               ...
               if(j==0){      
                 ...
                 if(tmptab[34].toString().equals("ACP") || tmptab[34].toString().equals("NCA")){
                    ...
                    if(indexcp==127)
                      ...
                      CheckCubicReccord. checkTva(con, erreur, CheckCubicReccord.ACHAT, tmptab);
           ...
           CheckCubicReccord.checkReccord(vente,CheckCubicReccord.ACHAT,erreur);
