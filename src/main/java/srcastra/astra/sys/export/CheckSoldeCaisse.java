...
public class CheckSoldeCaisse extends ExportMain implements ComptaExport{
   ...
   /*  String financier= 
    ...
    "AND h.jxcleunik = j.jota_cleunik AND (h.hetypeligne =  'BP' OR h.hetypeligne =  'CP' OR h.hetypeligne =  'P' OR h.hetypeligne =  'CPC') AND h.henotcpt = 1  AND h.heperiode = p.pede_cleunik AND h.typeintervenantcleunik = 2 AND t.lmcleunik=?  AND he_exported =0 AND cp.ce_cleunik=h.ce_cleunik2 ORDER BY h.henumpiece,h.hecleunik";  
    ...
    public CheckSoldeCaisse(int lmcleunik, Connection con, boolean bydossier, int pedecleunik,ArrayList erreur) throws SQLException{
    ...
    private void organize2() throws SQLException{
      ...
      for(int k=0;k<caisse.size();k++){
        ...
        Object[] main=(Object[])caisse.get(k);

        String toaffiche="";

        if(main[34].toString().equals("BP")){
            if(k==caisse.size()-1){
                CheckCubicReccord.checkFinancier(main,null, CheckCubicReccord.FINANCIER, erreur);                
            }
            else{
                Object[] obj=(Object[])caisse.get(k+1);
                if(!obj[34].toString().equals("CP")){
                    CheckCubicReccord.checkFinancier(main,null, CheckCubicReccord.FINANCIER, erreur);             
                }
                //BP(obj);
                CheckCubicReccord.checkFinancier(main, obj, CheckCubicReccord.FINANCIER, erreur);
            }
        } 
...
//       pstmt.setInt(2,pedecleunik);
    ...
    public java.util.ArrayList execute() throws SQLException {
         ...
         srcastra.astra.sys.utils.My_Logger.getLogger().info("CHECKING CAISSE");   
