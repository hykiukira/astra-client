 ...
 * @author  Thomas
 */
public class DiversPayementFournisseur extends TransfertPayement{
    
    /** Creates a new instance of ClientPayement */
    Caisselibelle_T caisse;
    String decre="";
    public DiversPayementFournisseur(Check check, Payement_T payement, Configuration config, Caisselibelle_T caisse) throws java.rmi.RemoteException, srcastra.astra.sys.rmi.Exception.ServeurSqlFailure, java.sql.SQLException{
    ...
    public void generatHistorique ()throws java.rmi.RemoteException,srcastra.astra.sys.rmi.Exception.ServeurSqlFailure,java.sql.SQLException{
        if(payement.getPrix()>0){
         ...
         //   if(!m_config.getDossier().isRattrap()){
            ...
            GetData.getCompteData(""+m_config.getM_urcleunik()).getSolde().add(new SoldeTot(Caisselibelle_T.FOURNISSEUR,new Long(m_config.getCleintervenant()).intValue(),payement.getPrix(),"D",null));
           ...
           // m_config.getM_serveur().renvSoldeRmiObject(m_config.getCle2()).insertDebit(m_config.getCle2(),Caisselibelle_T.FOURNISSEUR,new Long(m_config.getCleintervenant()).intValue(),payement.getPrix(),GetData.getCompteData().getPeriode(),payement,false);
        }
        ...
        //  if(!m_config.getDossier().isRattrap()){
            ...
            GetData.getCompteData(""+m_config.getM_urcleunik()).getSolde().add(new SoldeTot(Caisselibelle_T.FOURNISSEUR,new Long(m_config.getCleintervenant()).intValue(),Math.abs(payement.getPrix()),"C",null));
          ...
          //  m_config.getM_serveur().renvSoldeRmiObject(m_config.getCle2()).insertCredit(m_config.getCle2(),Caisselibelle_T.FOURNISSEUR,new Long(m_config.getCleintervenant()).intValue(),payement.getPrix(),GetData.getCompteData().getPeriode(),payement,false);
        }
       ...
       0,0,0,0,0,0);*/  
       check.base2(CheckCompteCentral.FOURNISSEUR,ParamComptableInterface.JOURNAL_ACHAT); 
        if(payement.getPrix()>0){
            decre="D";
          //if(!m_config.getDossier().isRattrap()){
            GetData.getCompteData(""+m_config.getM_urcleunik()).getSolde().add(new SoldeTot(Caisselibelle_T.GENE,GetData.getCompteData(""+m_config.getM_urcleunik()).getCe_cleunik(),payement.getPrix(),"D",null));
           // m_config.getM_serveur().renvSoldeRmiObject(m_config.getCle2()).insertDebit(m_config.getCle2(),Caisselibelle_T.GENE,GetData.getCompteData().getCe_cleunik(),payement.getPrix(),GetData.getCompteData().getPeriode(),payement,false);
            ...
            decre="C";
        //  if(!m_config.getDossier().isRattrap()){
            GetData.getCompteData(""+m_config.getM_urcleunik()).getSolde().add(new SoldeTot(Caisselibelle_T.GENE,GetData.getCompteData(""+m_config.getM_urcleunik()).getCe_cleunik(),Math.abs(payement.getPrix()),"C",null));
