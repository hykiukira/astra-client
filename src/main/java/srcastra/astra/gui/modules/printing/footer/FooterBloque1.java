...
public class FooterBloque1 extends AbstractBloque{
    ...
    protected void initField(int fcase, int color, int igenerique, String sgenerique, java.awt.Font font,ArrayList array) {
     ...
     //   Logger.getDefaultLogger().log(Logger.LOG_INFOS,"taille de la frame :"+size);//-(report.marginLeft+report.marginRight);
       // texteLegal.setruntimeValue("Quelques minutes avant le début des consultations à huis clos du Conseil de sécurité sur l'Irak, lundi après-midi 17 mars, les ambassadeurs américain, britannique et espagnol à l'ONU ont annoncé que Washington, Londres et Madrid avaient retiré leur projet de deuxième résolution qui visait à autoriser le recours à la force contre l'Irak. Pour expliquer ce retrait, l'ambassadeur britannique aux Nations unies, Jeremy Greenstock, a expliqué à la presse que le consensus au Conseil n'était pas possible");
        lieu=new MyRvfield(fcase,color,genprint.getAgence().getLocalité(),igenerique,sgenerique,font);
