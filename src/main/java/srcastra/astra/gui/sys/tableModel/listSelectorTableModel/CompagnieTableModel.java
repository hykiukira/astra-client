 ...
 * @author  Sébastien
 */
public class CompagnieTableModel extends AbstractAstraTableModel {
    
       
    /** Creates a new instance of CodePostauxTableModel */
    public CompagnieTableModel(astrainterface serveur, Loginusers_T currentUser) {
        super(serveur, currentUser);
    }
    
    protected String[] loadColumnNames() {
        try {
            java.util.ResourceBundle rb = java.util.ResourceBundle.getBundle("srcastra/astra/locale/tableHeader/signaletique/CompagnieTableHeader", currentUser.getLangage());
    ...
    protected ArrayList loadData() {
        ...
        try{
          retVal=serveur.renvSignalitiques(currentUser.getUrlmcleunik(),currentUser.getUrcleunik(),1,serveur.COMBO_COMPAGNIE,true);  
        }
        ...
        /* Temporaire ------------- */
       // retVal.add(new Object[] {"1", "SN", "SABENA" });
        //retVal.add(new Object[] {"2", "IB", "IBERIA" });
