 ...
 * @author  Sébastien
 */
public class AccomodationTableModel extends AbstractAstraTableModel {
    
       
    /** Creates a new instance of CodePostauxTableModel */
    public AccomodationTableModel(astrainterface serveur, Loginusers_T currentUser) {
        super(serveur, currentUser);
    }
    
    protected String[] loadColumnNames() {
        try {
            java.util.ResourceBundle rb = java.util.ResourceBundle.getBundle("srcastra/astra/locale/tableHeader/signaletique/AccomodationTableHeader", currentUser.getLangage());
        ...
        return new String[] {"Libele", "Abréviation"};
    }
    
    protected ArrayList loadData() {
        ArrayList retVal = new ArrayList(0);
        retVal.add(new Object[] { "1", "Hotel", "HO" });
        retVal.add(new Object[] { "2", "Bungalow", "BU" });
